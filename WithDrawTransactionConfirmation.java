...
public class WithDrawTransactionConfirmation extends JFrame implements Printable, ActionListener{
	...
	JLabel lblWithdrawAmount =new JLabel("");
	JLabel lblCurrentBalance =new JLabel("");
	JLabel lblDate = new JLabel("");
	JLabel lblEnd =new JLabel("----------------------------------------------");
	public WithDrawTransactionConfirmation(){
		...
		if(null!=ATMSession.getInstance().getItem("logedinwithdrawamnt")){
			withDrawAmount = ATMSession.getInstance().getItem("logedinwithdrawamnt").toString();
		}
	    ...
	    lblWithdrawAmount.setText("Your have done a withdwal of Rs:"+withDrawAmount);
	    lblCurrentBalance.setText("Your Current Balance is Rs:"+currentBalance);
	    lblDate.setText("Your Withdrawal date is:"+strDate);
	    pane.add(lblBank);
		pane.add(lblUser);
		pane.add(lblWithdrawAmount);
		...
		lblWithdrawAmount.setBounds(550,300,480,20);
	...
	public void actionPerformed(ActionEvent e){
		...
		if(source==btnPrint){
			...
			ATMSession.getInstance().removeItem("logedinwithdrawdate");
			ATMSession.getInstance().removeItem("logedincurrentbalance");
			ATMSession.getInstance().removeItem("logedinwithdrawamnt");
	...
	public int print(Graphics g, PageFormat pf, int page)
	    ...
	    if(null!=ATMSession.getInstance().getItem("logedinwithdrawdate")){
		 ...
		 if (page > 0) { /* We have only one page, and 'page' is zero-based */
	        ...
	        g.drawString("Your have done a withdwal of Rs:\t"+withDrawAmount, 100, 150);
	        
	        g.drawString("Your Current Balance is Rs:\t"+currentBalance, 100, 200);
	        
	        g.drawString("Your withdrawal date is:\t"+strDate, 100, 250);
